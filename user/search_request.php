        ...
        <main>
            ...
            <?php
                ...
                if (isset($_GET['search']) && isset($_GET['search_btn'])) {
                    ...
                    // select user from database
                    $select_request = mysqli_prepare($conn, "SELECT * FROM request_tbl WHERE description LIKE ? AND user_uuid = ? ORDER BY date_created DESC");
                    // declare search variable
                    $search_like = "%" . $search . "%";
                    // bind parameters 
                    mysqli_stmt_bind_param($select_request, "ss", $search_like, $_SESSION['uuid']);
                    // execute statement
