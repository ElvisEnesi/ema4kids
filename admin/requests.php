    ...
    <div class="dashboard_container">
        <aside>
            ...
            <div class="aside_links">
                ...
                <a href="<?= site_url ?>admin/requests.php" class="active"><ion-icon name="document-text-outline"></ion-icon> Requests</a>
        ...
        <main>
            <div class="main_navigator">
                ...
                <div class="search">
                    <form action="<?= site_url ?>admin/search_request.php" method="get">
                ...
                <div class="add">
                    ...
                    <button onclick="window.location.href='<?= site_url ?>admin/add_request.php'"><ion-icon name="add-outline"></ion-icon></button>
                </div>
            </div>
            <div class="filter_stat">
                <form action="<?= site_url ?>admin/requests_filter_decider.php" method="post">
                    <h4>Filter: </h4>
                    <div class="filt">
                        <button type="submit" name="all">All</button>
                        <?php
                            // select approved requests
                            $select_approved_requests = mysqli_prepare($conn, "SELECT COUNT(*) AS approved_requests FROM request_tbl WHERE status = ?");
                            // declare status
                            $approved_status = "approved";
                            // bind parameters
                            mysqli_stmt_bind_param($select_approved_requests, "s", $approved_status);
                            // execute statement
                        ...
                        <?php
                            ...
                            $select_pending_requests = mysqli_prepare($conn, "SELECT COUNT(*) AS pending_requests FROM request_tbl WHERE status = ?");
                            // declare status
                            $pending_status = "pending";
                            // bind parameters
                            mysqli_stmt_bind_param($select_pending_requests, "s", $pending_status);
                            // execute statement
                        ...
                        <?php
                            ...
                            $select_corrected_requests = mysqli_prepare($conn, "SELECT COUNT(*) AS corrected_requests FROM request_tbl WHERE status = ?");
                            // declare status
                            $corrected_status = "correct";
                            // bind parameters
                            mysqli_stmt_bind_param($select_corrected_requests, "s", $corrected_status);
                            // execute statement
                ...
                <form action="<?= site_url ?>admin/request_sort_decider.php" method="post">
                    <h4>Sort: </h4>
                    <div class="filt">
            ...
            <?php
                ...
                $select_request = mysqli_prepare($conn, "SELECT * FROM request_tbl ORDER BY date_created DESC");
                // execute statement
                ...
                <table>
                    ...
                    <?php while ($request = mysqli_fetch_assoc($select_request)) : ?>
                        <tr>
                            <td>
                                <?= htmlspecialchars($request['lastname'] . " " .  $request['firstname'] . " " . $request['middlename'] ?? null, ENT_QUOTES, "UTF-8") ?>
                            </td>
                            <td><?= decrypt(htmlspecialchars($request['encrypted_email'], ENT_QUOTES, "UTF-8")) ?></td>
                            <td><?= htmlspecialchars($request['description'], ENT_QUOTES, "UTF-8") ?></td>
                            <td><?= htmlspecialchars($request['currency'], ENT_QUOTES, "UTF-8"). "" . number_format(htmlspecialchars($request['total'], ENT_QUOTES, "UTF-8"), 2) ?></td>
                            <td>
                                <?= date("d M, Y", strtotime($request['date_needed'])) ?>
                            </td>
                            <td>
                                <?= date("d M, Y", strtotime($request['date_created'])) ?>
                            </td>
                            <td>
                                <a href="edit_status.php?id=<?= htmlspecialchars($request['id'], ENT_QUOTES, "UTF-8") ?>">
                                    Click
                                </a>
                            </td>
                            <td>
                                <?php if ($request['status'] === "approved") : ?>
                                    Unavailable
                                <?php else : ?>
                                    <a href="edit_request.php?id=<?= htmlspecialchars($request['id'], ENT_QUOTES, "UTF-8") ?>">
                                        Click
                                    </a>
                                <?php endif; ?>
                            </td>
