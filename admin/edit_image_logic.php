<?php
    ...
    $previous_avatar_path = "../images/admin/" . $previous_avatar;
    // get uuid from url
    if (isset($_GET['uuid'])) {
    ...
    if (isset($_POST['edit_image_ad'])) {
        // check if there's empty fields
        if (!$avatar['name']) {
            $_SESSION['edit_image_ad'] = "Select an image";
        } else {
            // work on new image
            $avatar_name = time() . "_" . $avatar['name']; // make image unique using timestamp
            $avatar_tmp_name = $avatar['tmp_name'];
            $avatar_destination = "../images/admin/" . $avatar_name;
            // allowed files
            ...
            if (in_array($mime_type, $allowed_files)) {
                ...
                if ($avatar['size'] > 3_000_000) {
                    $_SESSION['edit_image_ad'] = "File must be less than 3mb";
                }
            } else {
                $_SESSION['edit_image_ad'] = "Image must be jpg, jpeg or png";
            }
        }
        // redirect if there's any error
        if (isset($_SESSION['edit_image_ad'])) {
            header("Location: " . site_url . "admin/profile.php");
            exit();
        } else {
            ...
            $update = mysqli_prepare($conn, "UPDATE admin_tbl SET avatar = ? WHERE uuid = ?");
            // bind parameters
