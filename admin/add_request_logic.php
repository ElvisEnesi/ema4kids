<?php
    ...
    if (isset($_POST['request'])) {
        ...
        if (!$description || !$currency || !$total || !$date_of_request) {
            ...
            // echo $_SESSION['uuid'];
            $select = mysqli_prepare($conn, "SELECT uuid, firstname, lastname, middlename, encrypted_email FROM admin_tbl
            WHERE uuid = ? LIMIT 1");
        ...
        if (isset($_SESSION['add_request'])) {
            header("location: " . site_url . "admin/add_request.php");
            exit();
        } else {
            ...
            // bind parameters
            mysqli_stmt_bind_param($insert, "sssssssis", $details['firstname'], $details['lastname'],
            $middlename, $details['encrypted_email'], $description, $sanitized_date, $currency, $total, $_SESSION['uuid']);
            // execute statement
